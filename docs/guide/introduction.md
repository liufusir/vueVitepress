...
__结果展示__
<Demo :Dcontent="`Count is: ${count}`" @click="count++"/> 
...
## API 风格​
Vue 的组件可以按两种不同的风格书写：__选项式 API__ 和 __组合式 API__ 。
...
在学习阶段，你不必只固守一种风格。在接下来的文档中我们会为你提供一系列两种风格的代码供你参考，你可以随时通过左上角的 __API 风格偏好__ 来做切换。
